...
namespace DataShuttleInternals
{
	...
	template<class T, uint32_t Capacity>
	inline bool Buffer<T, Capacity>::TryPop(T & aOut)
	{
...
private:
	...
	static thread_local GrowingArray<DataShuttleInternals::Producer, uint32_t> ourProducers;
	static thread_local GrowingArray<DataShuttleInternals::Consumer, uint32_t> ourConsumers;
	...
	DataShuttleInternals::Buffer<T, BufferSizes>* myBuffers[MaxBuffers];
	uint8_t myPadding0[64 - (sizeof(void*) % 64)];
...
template <class T, uint32_t BufferSizes, uint32_t MaxBuffers>
thread_local GrowingArray<DataShuttleInternals::Producer, uint32_t> DataShuttle<T, BufferSizes, MaxBuffers>::ourProducers(4);
template <class T, uint32_t BufferSizes, uint32_t MaxBuffers>
thread_local GrowingArray<DataShuttleInternals::Consumer, uint32_t> DataShuttle<T, BufferSizes, MaxBuffers>::ourConsumers(4);
// -----------
...
inline void DataShuttle<T, BufferSizes, MaxBuffers>::InitProducer()
{
	...
	if (std::this_thread::get_id() != myOwnerId)
		ourProducers[myObjectId].myExit = std::bind(&DataShuttle::ExitProducer, this);	// RETHINK.. Need to make threads exiting de-claim buffer slots.
}

template<class T, uint32_t BufferSizes, uint32_t MaxBuffers>
inline void DataShuttle<T, BufferSizes, MaxBuffers>::ExitProducer()		// RETHINK
{
